                            ...
                            stage('SonarQube Scan') {
                                ...
                                if (env.SONAR_CREDENTIAL_ID != null && env.SONAR_CREDENTIAL_ID != '') {
                                    withCredentials([usernamePassword(credentialsId: "$SONAR_CREDENTIAL_ID", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                                        ...
                                        docker run --rm --user root -v "$WORKSPACE":/home/circleci/app $NODE_IMAGE /bin/bash -c "chown -R root:root /home/circleci/app && cd /home/circleci/app &&  npm install sonarqube-scanner -f &&  npm test -- --coverage && npm run sonar"
                                        sudo chown -R `id -u`:`id -g` "$WORKSPACE"
                                        """
                                    }
                                }
